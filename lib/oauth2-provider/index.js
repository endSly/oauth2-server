...
function getAccessToken(self, req, res) {
...
function authorize(self, req, res) {
  ...
  if('allow' in req.body || 'skip_allow' in req.body) {
    if('allow' in req.body)
      self.emit('client_allowed', req);
...
function tryAuthorize(self, req, res) {
  ...
  self.emit('enforce_login', req, res, authorize_url, function(user_id) {
    ...
    self.emit('authorize_form', req, res, client_id, authorize_url, function(){
      req.body.skip_allow = true;
      authorize(self, req, res);
    });
  });
};
